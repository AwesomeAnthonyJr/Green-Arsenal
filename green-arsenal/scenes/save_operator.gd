...
@onready var previous_value = SaveManager.player_save.seed_types[0]

func _ready() -> void:
	Dialogic.signal_event.connect(_on_dialogic_signal)
	if SaveManager.player_save.seed_types[0] == true:
...
func extra_stuff_in_physics_process():
	if SaveManager.player_save.seed_types[0] == true and just_picked_up:
		scene_index = operator_scenes.size() - 2
	elif SaveManager.player_save.seed_types[0] == false and has_interacted:
		scene_index = operator_scenes.size() - 1
